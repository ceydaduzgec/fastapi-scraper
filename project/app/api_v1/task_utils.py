...
from app.core.exceptions import NoFilesException, TaskNotFoundException
...
async def download_images(db, id, download_folder):
    ...
    if not download_task:
        ...
        raise TaskNotFoundException("Download task not found")
    ...
    os.makedirs(download_folder, exist_ok=True)  # TODO update for production
...
async def zip_images(db, id, download_folder):
    download_task = update_download_task_fields(db, id, {"status": DownloadStatus.ZIPPING})
    file_paths = [
    ...
    if not file_paths:
        raise NoFilesException("No files found to zip in the {downloads_folder} folder")
